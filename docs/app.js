const defaultApiBase = "https://ipaint.onrender.com";
...
const houseCenters = {1:{x:60,y:200},2:{x:130,y:130},3:{x:60,y:60},4:{x:200,y:60},5:{x:340,y:60},6:{x:270,y:130},7:{x:340,y:200},8:{x:270,y:270},9:{x:340,y:340},10:{x:200,y:340},11:{x:60,y:340},12:{x:130,y:270}};
...
function buildNorthIndianSvg(chartMap){
  ...
  const labels = Object.entries(houseCenters).map(([house,pos]) => {
    ...
    const planets = wrapText(data.planets || "", 15).map((line, idx) =>
      `<text class="svg-planets" x="${pos.x}" y="${pos.y + 20 + (idx * 14)}" text-anchor="middle">${escapeXml(line)}</text>`
    ).join("");
    return `<text class="svg-house-num" x="${pos.x}" y="${pos.y - 16}" text-anchor="middle">H${house}</text><text class="svg-sign" x="${pos.x}" y="${pos.y}" text-anchor="middle">${escapeXml(data.sign || "")}</text>${planets}`;
  }).join("");
  return `<svg viewBox="0 0 400 400" role="img" aria-label="North Indian chart">${lines}${labels}</svg>`;
}
...
function renderChartPayload(chart){
  ...
  Object.entries(chart.planets).forEach(([planet, info]) => {
    ...
    tr.innerHTML = `<td>${planet}${info.retrograde ? " (R)" : ""}</td><td>${info.sign}</td><td>${info.degree}°</td><td>${info.house}</td><td>${info.navamsa_sign} · H${info.navamsa_house}</td>`;
  ...
  chart.vimshottari.sequence.forEach(item => {
    ...
    div.innerHTML = `<div><strong>${item.lord} Mahadasha</strong><span>${item.start} → ${item.end}</span></div><span>${item.years} years</span>`;
...
async function createChart(payload){
  ...
  const response = await fetch(`${apiBase()}/api/chart`, {method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)});
  const data = await response.json();
  if(!response.ok) throw new Error(data.detail || "Could not generate the chart.");
...
demoBtn.addEventListener("click", async () => {
  ...
  try {
    await createChart({name:"Guest",date_of_birth:"13/05/1980",time_of_birth:"20:15",place_of_birth:"Pune, Maharashtra, India"});
  } catch (error) { setStatus(error.message, "error"); }
});

askBtn.addEventListener("click", async () => {
  if(!currentChart){ setStatus("Generate the kundli first.", "error"); return; }
  ...
  try {
    const response = await fetch(`${apiBase()}/api/ask`, {method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({question, chart_context: currentChart})});
    const data = await response.json();
